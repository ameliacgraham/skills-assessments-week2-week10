...
# flask-sqlalchemy object (in other words a query object because nothing is
#                          executed until .all(), .(get) etc. is called)
...
# An association table is a table that connects two tables, and has
# no other relevant data columns. It manages a many to many relationship.
...
def search_brands_by_name(mystr):
    ...
    brands_by_name = Brand.query.filter(Brand.name.like('%{}%'.format(mystr))).all()
...
def get_models_between(start_year, end_year):
    ...
    models_between_years = Model.query.filter(Model.year > start_year, Model.year < end_year).all()
